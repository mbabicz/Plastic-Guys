...
public class ArmsController : MonoBehaviour
{
    ...
    [SerializeField] private ConfigurableJoint armJoint;
    [SerializeField] private Transform rightArm;
    private Quaternion startingRotation;

    void Start()
    {
        //joint = upperArm.gameObject.GetComponent<ConfigurableJoint>();
        //startingRotation = Quaternion.Inverse(upperArm.localRotation);
    }


    private void Update() {
        if(Input.GetMouseButtonDown(0)){//left arm
           armJoint.targetRotation = upperArm.localRotation * startingRotation;
        }
    }
